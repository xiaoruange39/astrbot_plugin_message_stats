...
- **修复 Telegram 平台头像显示错乱**：`_get_avatar_url()` 方法硬编码使用腾讯 `q1.qlogo.cn` 头像服务，导致 Telegram 用户的 ID 被发送到 QQ 头像服务，返回随机其他人头像或 QQ 默认图标。已修复：彻底移除第三方头像服务依赖，无用户头像时自动显示基于昵称首字的彩色圆形头像（哈希分配15色之一），跨平台兼容 QQ、Telegram、Discord 等所有平台。
- **修复里程碑卡片缺失头像时显示空图**：里程碑 HTML 模板中的头像 `<img>` 在链接无效时显示空白。已修复：模板增加条件渲染，无头像 URL 时同样显示彩色首字母圆形头像。

### 🔧 其他改进

- `_get_avatar_url()` 方法不再绑定任何平台，始终返回空字符串（由模板负责优雅回退）

### ✨ 新功能

- **插件国际化支持（zh-CN/en-US/ru-RU）**：完善了 WebUI 中插件名称、描述和配置项的翻译。新增俄语翻译文件 `ru-RU.json`。
- **Page 页面前端多语言**：页面根据浏览器语言自动检测切换中文/英文/俄语，包括标题、按钮、提示等全部文案。
- **图片排行榜多语言**：新增 `image_language` 配置项（zh-CN/en-US/ru-RU），可在 WebUI 插件设置中选择排行榜图片的文字语言。
- **里程碑卡片多语言**：里程碑成就卡片支持中文/英文/俄语显示。
- **Page 页面头衔颜色**：Web 面板中的头衔徽章现在使用 `title_color` 动态颜色，与排行榜图片风格一致。

### 🔧 其他改进

- LLM Token 消耗文本按语言翻译
- 所有排行榜模板和后端回退方案的硬编码中文均已替换为可配置的多语言变量
