...
### ✨ 新功能
- **排行榜跨周期趋势**：周期榜支持显示相比昨天、上周、上月、去年等上一周期的发言数变化和名次变化；新上榜成员显示“相比上一周期新上榜”
- **总榜近期活跃趋势**：总榜支持显示近期趋势上升、下降或平稳，统计天数可配置（默认近 7 天）
- **趋势显示开关**：新增 `rank_trend_enabled` 配置，可一键开启或关闭排行榜趋势信息

### 🎨 界面改进
- 趋势文案从单独的箭头和百分比改为语义化文本，例如“相比昨天下降17% · 名次上升2位”
- 趋势信息移动到昵称和最近发言信息区域，避免右侧统计栏拥挤和文字断行
- 文字、图片、PDF 及各排行榜主题统一显示趋势信息

### 🐛 Bug 修复
- **定时推送缺少趋势文案**：定时/手动推送路径现在也会显示周期对比和总榜近期趋势；今日榜无数据回退到昨日榜时，比较基准同步改为前天
