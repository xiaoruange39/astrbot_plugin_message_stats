...
class Validators:
    ...
    def validate_nickname(nickname: Any) -> str:
        ...
        # 使用HTML转义而不是简单的字符黑名单
        import html
        escaped_nickname = html.escape(nickname_str)
        
        # 检查转义后的长度是否异常（防止编码攻击）
        if len(escaped_nickname) > len(nickname_str) * 2:
            raise ValidationError("昵称包含可疑的编码字符")
        
        return escaped_nickname
    ...
    def validate_file_path(file_path: str, allowed_extensions: Optional[List[str]] = None, 
        ...
        # 使用os.path.realpath进行路径规范化，解析符号链接和相对路径
        try:
            # 首先规范化路径
            normalized_path = os.path.abspath(file_path)
            # 使用realpath解析符号链接和相对路径引用
            real_path = os.path.realpath(normalized_path)
            
            # 验证路径是否一致（防止路径遍历攻击）
            if normalized_path != real_path:
                # 如果realpath改变了路径，可能存在符号链接攻击
                # 检查realpath是否在允许的目录内
                if allowed_base_path:
                    ...
                    if not real_path.startswith(allowed_real_path + os.sep) and real_path != allowed_real_path:
                        raise ValidationError("文件路径可能包含符号链接攻击")
        ...
        if allowed_base_path:
            try:
                ...
                allowed_real_path = os.path.realpath(allowed_abs_path)
                # 确保路径在允许的目录内
                if not final_path.startswith(allowed_real_path + os.sep) and final_path != allowed_real_path:
        ...
        if allowed_extensions:
            _, ext = os.path.splitext(final_path.lower())
