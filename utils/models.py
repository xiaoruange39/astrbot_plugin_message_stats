...
async def load_json_file(file_path: str) -> Optional[Dict[str, Any]]:
    ...
    异步读取JSON文件并解析内容，包含完整的错误处理机制。
    
    Args:
        file_path (str): JSON文件路径
        
    Returns:
        Optional[Dict[str, Any]]: 解析后的字典数据，如果加载失败返回None
        
    Raises:
        FileNotFoundError: 当文件不存在时（被捕获，返回None）
        json.JSONDecodeError: 当JSON格式错误时（被捕获，返回None）
        IOError: 当文件读取失败时（被捕获，返回None）
        OSError: 当系统操作失败时（被捕获，返回None）
...
async def save_json_file(file_path: str, data: Dict[str, Any]) -> bool:
    ...
    Returns:
        bool: 保存是否成功，True表示成功，False表示失败
    ...
    Example:
        ...
        >>> success = await save_json_file("output.json", data)
